...
public class CustomSimilarity extends Similarity {
	 ...
	 *             if {@code k1} is infinite or negative, or if {@code b} is not
	 *             within the range {@code [0..1]}
	 */
	public CustomSimilarity(float k1, float b) {
	 ...
	 * </ul>
	 */
	public CustomSimilarity(IndexReader reader) {
		this(1.2f, 0.75f);
		this.distance = DEFAULT_DISTANCE;
		this.reader = reader;
	}

	public CustomSimilarity(IndexReader reader, int distance) {
		this(1.2f, 0.75f);
	...
	/**
	 * Implemented as
	 * <code>log(1 + (docCount - docFreq + 0.5)/(docFreq + 0.5))</code>.
	 */
	 ...
	 * <code>sumTotalTermFreq / docCount</code>
	 */
	...
	/**
	 * Sets whether overlap tokens (Tokens with 0 position increment) are ignored
	 * when computing norm. By default this is true, meaning overlap tokens do not
	 * count when computing norms.
	 */
	...
	/**
	 * Computes a score factor for a simple term and returns an explanation for that
	 * score factor.
	 * 
	 * <p>
	 ...
	 * because also {@link TermStatistics#docFreq()} is used, and when the latter is
	 * inaccurate, so is {@link CollectionStatistics#docCount()}, and in the same
	 * direction. In addition, {@link CollectionStatistics#docCount()} does not skew
	...
	public Explanation idfExplain(CollectionStatistics collectionStats, TermStatistics termStats) {
		...
		currentTerm = termStats.term();

		String term = new String(currentTerm.bytes);
		final long docCount = collectionStats.docCount() == -1 ? collectionStats.maxDoc() : collectionStats.docCount();
		final float idf = idf(df, docCount);
		return Explanation.match(idf,
				term + "//idf, computed as log(1 + (docCount - docFreq + 0.5) / (docFreq + 0.5)) from:",
				Explanation.match(df, "docFreq"), Explanation.match(docCount, "docCount"));
	}
	...
	public final SimScorer simScorer(SimWeight stats, LeafReaderContext context) throws IOException {
		...
		return new BM25DocScorer(bm25stats, context.reader().getMetaData().getCreatedVersionMajor(),
	...
	private class BM25DocScorer extends SimScorer {
		...
		private BytesRef getTermFromIdfDescription(String description) {
			if (description.contains("//")) {
				String term = description.split("//")[0];
				return new BytesRef(term.getBytes());
			}
			return null;
		}
		...
		public float score(int doc, float freq) throws IOException {
			...
			Terms terms = reader.getTermVector(doc, Constants.CONTENT);
			int pos = 0;
			if (terms.hasPositions()) {
				TermsEnum termsEnum = terms.iterator();
				PostingsEnum postings = null;
				termsEnum.seekExact(getTermFromIdfDescription(stats.idf.getDescription()));
			...
			float result;
			if (pos > distance) {
				result = weightValue * freq / (freq + norm);
			} else {
				result = 3 * weightValue * freq / (freq + norm);
			}
			return result;
		}
	...
	private static class BM25Stats extends SimWeight {
		...
		/**
		 * precomputed norm[256] with k1 * ((1 - b) + b * dl / avgdl) for both
		 * OLD_LENGTH_TABLE and LENGTH_TABLE
		 */
